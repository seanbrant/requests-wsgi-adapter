...
class WSGIAdapterTest(unittest.TestCase):
    ...
    def test_plain_text(self):
        response = self.session.post(
            "http://localhost/index", data="Once upon a time..."
        )
...
class WSGIAdapterCookieTest(unittest.TestCase):
    ...
    def test_delete_cookies(self):
        ...
        session.mount("http://localhost/cookies/set", WSGIAdapter(app=set_app))
        session.mount("http://localhost/cookies/delete", WSGIAdapter(app=delete_app))

        session.get("http://localhost/cookies/set?flimble=floop&flamble=flaap")
        self.assertEqual(session.cookies["flimble"], "floop")
        self.assertEqual(session.cookies["flamble"], "flaap")

        session.get("http://localhost/cookies/delete?flimble")
